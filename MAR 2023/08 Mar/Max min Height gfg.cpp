    ...
    public:
        bool isOK(vector<int> &arr, int days, int cs, long long int h)
        {
            ...
            for(int i = 1; i < arr.size(); i++)
            {
                ...
                if(i>=cs) actualHeight += (waterSupply[i]-waterSupply[i-cs]);
        ...
        {
            long long res = -1;
            long long mnHeight = *min_element(a.begin(),a.end());
            long long mxHeight = INT_MAX/2;
